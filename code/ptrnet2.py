...
class PtrNet(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(self.seq_len):
            ...
            #current_input: batch_size*seq_len*hidden_dim -> batch_size*hidden_dim
            current_input = torch.sum(torch.mul(a_i, encoding_hidden_states), 1)
...
class Trainer:
    def __init__(self, batch_size, seq_len, input_dim, hidden_dim, learning_rate,from_former_model):
        ...
        self.optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, self.ptrNet.parameters()), lr=learning_rate)
        self.CEL = torch.nn.CrossEntropyLoss()
        self.episode = 0
        self.filename = "Models/tsp_net"+"_batch_size="+str(self.batch_size)+"_hidden_dim="+str(self.hidden_dim)+\
                   "_seq_len="+str(self.seq_len)+"learn_rate="+str(self.learning_rate)+"wssb.pt"
    ...
    def train(self, input, ground_truth):
        ...
        calc_output=output.reshape((self.batch_size*self.seq_len,self.seq_len))
    ...
    def check_result(self,input,output,truth):
        ...
        for case in range(input.shape[0]):
            ...
            for i in range(1,input.shape[1]):
                ...
                tmp_optimal_length+=torch.sqrt(torch.sum(torch.pow(input[case][truth[case][i]]-input[case][truth[case][i-1]],2)))
            tmp_tour_length+=torch.sqrt(torch.sum(torch.pow(input[case][tour[0]]-input[case][tour[input.shape[1]-1]],2)))
            tmp_optimal_length+=torch.sqrt(torch.sum(torch.pow(input[case][truth[case][0]]-input[case][truth[case][input.shape[1]-1]],2)))
            if tmp_tour_length-tmp_optimal_length<0:
                print(input[case])
                print(tour)
                print(truth[case])
...
trainer = Trainer(BATCH_SIZE, SEQ_LEN, INPUT_DIM, HIDDEN_DIM, LEARNING_RATE,True)
