...
class PtrNet(nn.Module):
    ...
    def forward(self, input):
        ...
        output = []
        selection=[]
        mask=torch.ones([self.batch_size,self.seq_len],requires_grad=False).bool()
        num=torch.tensor([i for i in range(self.seq_len)])
        num=num.unsqueeze(0).expand([self.batch_size,self.seq_len])
        if USE_CUDA:
            mask=mask.cuda()
            num=num.cuda()
        ...
        for i in range(self.seq_len):
            ...
            u_i.masked_fill_(mask.logical_not(), -float("inf"))
            ...
            selection.append(chosen_value)
            chosen_value=chosen_value.unsqueeze(1).expand([self.batch_size,self.seq_len])

            # one_hot: batch_size*seq_len
            one_hot=chosen_value.eq(num)
            # print("one_hot",i,one_hot[0])
            mask=mask.logical_xor(one_hot)

            # one_hot-> batch_size*seq_len*input_dim
            one_hot=one_hot.unsqueeze(2).expand([self.batch_size,self.seq_len,self.input_dim])
            # print(one_hot.size())
            # print(input.size())
            #current_input: batch_size*input_dim
            current_input=input.masked_select(one_hot).reshape([self.batch_size,self.input_dim])
        ...
        selection=torch.stack(selection).transpose(0,1)
        # return: seq_len*batch_size -> batch_size*seq_len
...
class Trainer:
    def __init__(self, batch_size,input_dim, hidden_dim, encoder_layers, learning_rate, from_former_model):
        ...
        self.episode = 0
        self.seq_len = 0
        self.filename = "10mydata_masked.pt"
        if from_former_model:
            self.load_model()
        self.outf=open("10result_masked.txt","w")

    def train(self, input, ground_truth):
        self.seq_len = input.shape[1]
        batch_size=input.shape[0]
        output ,selection= self.ptrNet(input.float())
    ...
    def check_result(self, input, output, truth):
        ...
        average_ans=tour_length/batch_size
        tour_length/=batch_size
        optimal_length/=batch_size
        ...
        return str(score)+" "+str(average_ans.cpu().numpy())+" "+str(optimal_length.cpu().numpy())
